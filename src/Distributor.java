...
public class Distributor {
	...
	public static void distribute(int hosts) throws IOException
	{
		int counter = 0;
	...
	public static void measureSize() throws IOException
	{
		int counter = 0;
		File file = new File("access.log.3.replay");
		List<Long> iplist = new ArrayList<Long>();
		...
		List<String> receivedLog = new ArrayList<String>();
		...
		try {
			...
			while (sc.hasNextLine()) {
				...
				receivedLog.add(line+";"+content_length);
			...
			try {
				PrintWriter writer;
				writer = new PrintWriter("output/measuredHeader.txt","UTF-8");

				for(int j = 0 ; j < receivedLog.size();++j)
				{
					writer.println(receivedLog.get(j));
				}
				writer.close();				
			} catch (UnsupportedEncodingException e) {
				// TODO Auto-generated catch block
